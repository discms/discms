...
import { GISCUS_APP_HOST } from 'lib/services/config';
import { getCategories } from 'lib/services/giscus/categories';
    ...
    <div className="p-4 pt-0 markdown">
      ...
      <fieldset>
        ...
        <input
          ...
          onChange={event => setConfig(current => ({ ...current, repository: event.target.value }))}
          type="text"
          ...
          <div key={value} className="mt-4 first:mt-0 form-checkbox">
            ...
            {['specific', 'number'].includes(config.mapping) && config.mapping === value ? (
              <input
                id="term"
                value={config.term}
                onChange={event => setConfig(current => ({ ...current, term: event.target.value }))}
      ...
      <div className="form-checkbox">
        <input
          ...
          onChange={event => setConfig(current => ({ ...current, lazyLoad: event.target.checked }))}
        ></input>
